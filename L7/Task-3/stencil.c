...
// Smaller STENCIL_SZ gives fewer iterations in the innermost loop, reducing the overall time for all functions
// but the fused loops are still faster.
...
void apply_stencil_v3(const GridPt* grid_src, GridPt* grid_dst, int N) {
  ...
  const int blockSz = 4; // NOTE: CHOOSE BLOCKSIZE HERE! (blockSz = N_eff is like no blocking)
  if(N_eff % blockSz != 0) {
