...
void processNeighbours(int** dest, int** all, int x, int y, int z, char*** test_byte);
...
int main(int argc, char* argv[]){
	...
	char*** test_byte = generateVolume(35,35,35);
	char*** new_test_byte = generateZeroVolume(size,size,size);
//	printf("Initial state: \n");
//	printVolume(states, size, size, size);
	int** alive = findAlive(test_byte,size,size,size);

	int** all = generateCoordinateArrays(size*3, size*3, size*3);
	processNeighbours(all, alive, size,size,size, test_byte);
...
//					new_test_byte[index] = 1;
		...
		processNeighbours(all, updated, size, size, size, test_byte);
...
void processNeighbours(int** dest, int** alive,  int x, int y, int z, char*** test_byte){
	double time = get_wall_seconds();
	int** all = dest;
...
//	printf("Allocated %li bytes for neighbors\n", sizeof(int)*alive[0]*27+4);
	...
	for(int i = 0; i < limit; i++){
		int z_coord = z_axis[i];
		int y_coord = y_axis[i];
		int x_coord = x_axis[i];
