...
//	printf("Input read in %lf s\n", get_wall_seconds()-time);
	...
	for(int i = 0; i < n_threads; i++){
		double* sub_acc = (double*) malloc(sizeof(double)*n_particles*2);
...
void acceleration(double* data_array, double* acc_array, int n, int n_threads, double** sub_acc_arrays){
	...
	int i, j;
	double denom;
	//double* sub_acc = (double*) malloc(sizeof(double)*n*2);
	//sub_acc_arrays[omp_get_thread_num()] = sub_acc;
	double* sub_acc = sub_acc_arrays[omp_get_thread_num()];
	memset(sub_acc, 0, 2*8*n);

	for(i = 6*omp_get_thread_num(); i < 6*n; i +=6*omp_get_num_threads()){
		...
		for(j = 0; j < i; j+=6){
			...
			sub_acc[2*(i/6)] += -m_j*(x_i-x_j)*denom;
			sub_acc[2*(i/6)+1] += -m_j*(y_i-y_j)*denom;
			sub_acc[2*(j/6)] += m_i*(x_i-x_j)*denom;
			sub_acc[2*(j/6)+1] += m_i*(y_i-y_j)*denom;
	...
	for(int i = 0; i < n_threads; i++){
		for(int j = 0; j<2*n; j++){
			if (i==0){
				acc_array[j] = sub_acc_arrays[i][j];
			} else{
				acc_array[j] += sub_acc_arrays[i][j];
			}
		}
